...
class AuthService:
    ...
    def login(self, username: str, password: str) -> User:
        ...
        # 1. Формальная проверка существования и активности
        # (задержка чтобы не дать timing attack на username)
        if not user or not user.is_active:
            # Делаем dummy check для защиты от timing-атаки
            dummy_hash = b"$2b$12$KIXeW3H0g0Z3M5.dJ/H95ui5X2E5jI2G6l3Q1M9S9Zp/y7o0aB1X."
        ...
        if not bcrypt.checkpw(password.encode("utf-8"), user.password_hash.encode("utf-8")):
            ...
            logger.warning(
                ...
                f"({user.login_attempts}/{MAX_LOGIN_ATTEMPTS})"
            )

            if user.login_attempts >= MAX_LOGIN_ATTEMPTS:
                logger.error(
                    f"Учётная запись заблокирована из-за множества неудачных попыток: {username}"
                )
        ...
        app.current_user = user # type: ignore
    ...
    def logout(self) -> None:
        user = app.current_user # type: ignore
        if user:
            logger.info(f"Выход из системы: {user.username}")
            app.current_user = None # type: ignore
