# ui/task_constructor/editors/multi_choice.py
...
class MultiChoiceEditor(AbstractTaskEditor):
    """
    Редактор для задания типа "Несколько ответов" (multi_choice).
    Позволяет добавлять варианты ответов с чекбоксами для выбора нескольких верных.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        self.options = [] # Список dict: {"id": str, "widget": QWidget, "checkbox": QCheckBox, "input": QLineEdit}
    ...
    def _add_empty_option(self, is_correct=False):
        ...
        checkbox.setChecked(is_correct)
        checkbox.toggled.connect(lambda: self.data_changed.emit())
    ...
    def get_specific_data(self) -> dict:
        ...
        for opt in self.options:
            formatted_options.append({
                ...
                "is_correct": opt["checkbox"].isChecked()
            })
